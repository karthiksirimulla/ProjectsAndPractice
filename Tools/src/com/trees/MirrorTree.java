package com.trees;

import java.util.Stack;

import com.node.TreeNode;

public class MirrorTree
{
    public static void main( String[] args )
    {
        TreeNode rootNode = constructTree();
        preOrder( rootNode );
        mirrorTree1( rootNode );
        preOrder( rootNode );

    }

    private static TreeNode mirrorTree( TreeNode Node )
    {
        if( Node == null )
            return Node;
        System.out.println( "-->" + Node.data );
        TreeNode left = mirrorTree( Node.left );
        TreeNode right = mirrorTree( Node.right );
        System.out.println( "##" + Node.data + " Left : " + Node.left + " Right :" + Node.right );
        Node.left = right;
        Node.right = left;

        return Node;

    }

    private static void mirrorTree1( TreeNode Node )
    {
        if( Node == null )
            return;
        System.out.println( "-->" + Node.data );
        mirrorTree1( Node.left );
        mirrorTree1( Node.right );
        if( Node.left != null && Node.right != null )
            System.out.println( "##" + Node.data + " Left : " + Node.left.data + ", Right :" + Node.right.data );
        else
            System.out.println( "##" + Node.data + " Left : " + Node.left + ", Right :" + Node.right );

        TreeNode temp = Node.left;
        Node.left = Node.right;
        Node.right = temp;

        System.out.println( "---- After SWAP -----" );

        if( Node.left != null && Node.right != null )
            System.out.println( "##" + Node.data + " Left : " + Node.left.data + ", Right :" + Node.right.data );
        else
            System.out.println( "##" + Node.data + " Left : " + Node.left + ", Right :" + Node.right );

    }

    private static void preOrder( TreeNode root )
    {
        Stack<TreeNode> stack = new Stack<>();
        stack.push( root );

        while( !stack.isEmpty() )
        {
            TreeNode temp = stack.pop();
            System.out.print( temp.data + ", " );

            if( null != temp.right )
                stack.push( temp.right );
            if( null != temp.left )
                stack.push( temp.left );
        }
        System.out.println();
    }

    private static TreeNode constructTree()
    {
        /*
         *                1
                     2         3
                  4     5    6    7
                  
                          
         */

        TreeNode node1 = new TreeNode( 1 );
        TreeNode node2 = new TreeNode( 2 );
        TreeNode node3 = new TreeNode( 3 );
        TreeNode node4 = new TreeNode( 4 );
        TreeNode node5 = new TreeNode( 5 );
        TreeNode node6 = new TreeNode( 6 );
        TreeNode node7 = new TreeNode( 7 );

        node1.left = node2;
        node1.right = node3;

        node2.left = node4;
        node2.right = node5;

        node3.left = node6;
        node3.right = node7;

        return node1;

    }

}
